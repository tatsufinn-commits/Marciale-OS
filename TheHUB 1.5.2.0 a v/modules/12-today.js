...
function renderRuViewPage(){
  // FIXED (Build 32 perf): Skip all work if RuView page isn't active
  const ruviewPage = document.getElementById('page-ruview');
  if(ruviewPage && !ruviewPage.classList.contains('active')) return;

  // === BUILD 32: RuView Bridge Status ===
  const bannerEl = document.getElementById('ruviewBanner');
  const bannerIcon = document.getElementById('ruviewBannerIcon');
  const bannerTitle = document.getElementById('ruviewBannerTitle');
  const bannerSub = document.getElementById('ruviewBannerSub');
  const bannerBadge = document.getElementById('ruviewBannerBadge');
  const sensingEl = document.getElementById('ruviewSensingStatus');
  const sensingDetail = document.getElementById('ruviewSensingDetail');
  const signalEl = document.getElementById('ruviewSignalField');
  const motionEl = document.getElementById('ruviewMotionChart');
  const vitalsEl = document.getElementById('ruviewVitals');
  const connStatsEl = document.getElementById('ruviewConnectionStats');

  const bridgeConnected = typeof ruviewIsConnected === 'function' && ruviewIsConnected();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Build 32: RuView bridge controls
  document.getElementById('ruviewConnectBtn')?.addEventListener('click', () => {
    if(typeof connectRuView === 'function') connectRuView();
    renderRuViewPage();
  });
  document.getElementById('ruviewDisconnectBtn')?.addEventListener('click', () => {
    if(typeof disconnectRuView === 'function') disconnectRuView();
    renderRuViewPage();
  });
