...
**Authority:** Supreme Commander (Director) & ASSISTANT (Chief Operating Officer)  
**Parent Governance:** `/docs/AI_RULES.md` (Laws I through XIII)  
**Classification:** TIER 1 GOVERNANCE BLUEPRINT  
...
┌─────────────┐  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐  ┌─────────────┐
│    [ J ]    │  │    [ R ]    │ │    [ W ]    │ │    [ E ]    │  │    [ N ]    │
│    JOINT    │  │RECONNAISSANCE││   WISDOM    │ │  ENGINEER   │  │  NAVIGATOR  │
├─────────────┤  ├─────────────┤ ├─────────────┤ ├─────────────┤  ├─────────────┤
│ Syndicate   │  │ Outside-    │ │ Prompt      │ │ Heavyweight │ │ Cross-Repo  │
│ Protocol &  │  │ World Tech  │ │ Architecture│ │ Code, Canvas│ │ Cartographer│
│ Handshake   │  │ Intel & APIs│ │ & Governance│ │ & Game Math │ │ & Spec Lead │
└─────────────┘  └─────────────┘ └─────────────┘ └─────────────┘  └─────────────┘
```
...
|---|:---:|---|
| **`ASSISTANT` (Seat A)** | **ACTIVE & RATIFIED** | 43 test suites verified 100% green; Build 39 & 40 delivered; Web Audio engine implemented; iframe origin security hardened; `@pangolin` sentinel built. Recorded in `BUILD_LOGBOOK.md` and `PATCHNOTES_LEDGER.md`. |
| **`WISDOM` (Seat W)** | **INVITED (Active Partner)** | Authored the Governance Integrity Audit directive; pioneered the 3-Truth Model; established the 9-Point Repair Review; authored high-leverage scenario prompts. |
| **`RECONNAISSANCE` (Seat R)** | **CHARTERED (Pending Commission)** | Evaluated against benchmark dossier quality, licensing audits, and procedural tech scouting accuracy. |
| **`ENGINEER` (Seat E)** | **CHARTERED (Pending Commission)** | Evaluated against Canvas 2D performance, mathematical precision, and 100% passing test assertion rates. |
| **`NAVIGATOR` (Seat N)** | **CHARTERED (Pending Commission)** | Evaluated against cross-subsystem contract synchronization, schema validation, and WCAG 2.2 AA audits. |
