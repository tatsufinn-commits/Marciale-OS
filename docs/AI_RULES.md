...
I am a beginner programmer, so you must assume the role of [AGENT ROLE, e.g. @architect | @sentinel | @forge | @mind | @sre] per `docs/AGENTS.md` and strictly follow our AI Constitution in `docs/AI_RULES.md`.
...
2. Follow the 6 Laws in docs/AI_RULES.md (no rewrites, local-first, simulation mode for hardware).
3. Modify only the specific files needed for this single task.
4. Run `npm test` and make sure all 43 tests pass before finishing.
5. Explain what you did in simple beginner terms so I understand!
```
